...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 运营管理 <span class="c-gray en">&gt;</span> 约伴活动 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container"><form action="" method="get">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" id="delete" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>批量删除</a> <a href="javascript:;"  data-title="添加活动" onclick="activity_add('添加活动','/admin/FreedomActivity/add',800,800)" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加活动 </a></span> <span class="r">共有数据：<strong><?php echo $total;?></strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg">
		<thead>
			<tr class="text-c">
				...
				<th width="200">活动地址</th>
				<th width="130">活动开始时间</th>
				<th width="70">出行方式</th>
				<th width="80">活动状态</th>
				<th width="130">活动人数</th>
				<th width="70">发布时间</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		...
		<tbody>
			<tr class="text-c">
				...
				<td><?php echo $val['province'].$val['city'].$val['county'].$val['address']?></td>
				<td><?php echo date("Y-m-d H:i:s",$val['start_time'])?></td>
				<td><?php echo $val['go_type']?></td>
				<td><?php if($val['status'] == 1){echo '进行中';}elseif($val['status'] == 0){echo '未开始';}elseif($val['status'] == 2){echo '已结束';}elseif($val['status'] == -1){echo '禁用';}?></td>
				<th><?php echo $val['participants']?> / <?php echo $val['need']?></th>
				<th width="70"><?php echo date("Y-m-d H:i:s",$val['create_time'])?></th>
				<td class="td-manage"><?php if($val['status'] != -1){?><a style="text-decoration:none" onClick="member_stop(this,'<?php echo $val['id']?>')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a><?php } if($val['status'] == -1){?> <a style="text-decoration:none" onClick="member_start(this,'<?php echo $val['id']?>')" href="javascript:;" title="启用"><i class="Hui-iconfont"><i class="Hui-iconfont">&#xe6e1;</i></i> </a><?php }?><a title="编辑" href="javascript:;" onclick="activity_edit('修改活动','/admin/FreedomActivity/edit?id=<?php echo $val['id']?>',800,800)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
