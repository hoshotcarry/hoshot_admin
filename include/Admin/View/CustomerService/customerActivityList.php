...
<div class="page-container"><form action="" method="get">
	<div class="text-c"> 日期范围：
		...
		<input type="hidden" class="input-text" id="" name="customer_id" value="<?php  echo $arr[0]['id'];?>" name="search">
		<button type="submit" class="btn btn-success radius" id="" name="" value=""><i class="Hui-iconfont">&#xe665;</i> 查询 </button>
	</div></form>
		
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		    <span  id="" >
	    	<a href="javascript:;" id="" class="btn btn-success radius">客服：【<?php  if ($arr[0]['account']) { echo $arr[0]['account']; ?>  】的活动管理列表 <?php }else{ echo "亲，该客服无数据哦!】"; }?></a>
	    </span>
...
// 		alert(index);
		...
		//&customer_id="+selected_val;//"id="+delID+"&
		//alert(datas);
 		$.ajax({
			   type: "POST",
		 ...
		 /*  $.post('/admin/Activity/joinActivity',{id:delID,customer_id:selected_val},function(data){
            alert(data)
           $.each(index,function(m,n){
            	$("#del-"+n).parents("tr").remove();
            });       
            alert(data);    
		    layer.msg('已加入!',{icon:1,time:1000});
        },'json');*/ 	
	//});
