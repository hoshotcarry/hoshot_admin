...
//     			)
    ...
    public function sub_back($id){
    	...
    	//修改用户活动状态为-1
//     	$ret = M('user_activity')->where(['user_id'=>$back['user_id'] ,'act_id'=>$id,'status'=>1,'order_id'=>$back['id']])->setField('status',-1);
    	$backStatus = $model->where(array('id'=>$id))->save(array('status'=>1,'create_time'=>time()));
