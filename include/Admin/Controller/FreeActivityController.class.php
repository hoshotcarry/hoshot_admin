...
class FreeActivityController extends ConmmonController {
    ...
    public function index(){
        ...
        $map = array();
        $map[C('DB_PREFIX').'activity.activity_price'] = 0;
        $map[C('DB_PREFIX').'activity.channel_id'] = array('NEQ',$this->channel_id('约伴游'));
        ...
        if(IS_GET){
        	...
        	if(!empty($_GET['start_time']) and !empty($_GET['end_time'])){
        		$map[C('DB_PREFIX').'activity.create_time'] = array(array('egt',strtotime($_GET['start_time'])),array('elt',strtotime($_GET['end_time'])));
        	}
...
//         		$map['_complex'] = $where;
    ...
    public function channel_id($name){
    	$row = M('category')->field('id')->where(array('title'=>$name,'status'=>1))->find();
    	return $row['id'];
    }
    ...
    /* 
     * carry 2016-12-26  update  编辑时可以上传图片
     *  */
    		...
    		if(false !== $model->create()){
    			...
    			if($edit){
    				...
    				if(!empty($_POST['postion']))
    					...
    					// (!empty($_FILES['file'.($index+1)]['name'])?$this->create_images_path(10000001,$_FILES['file'.($index+1)]): '')
    				if(!empty($_POST['img'])){
