...
//     			)
    ...
    public function sub_back($id){
    	...
    	$back = M("order_back")->where(array('id'=>$id))->find();
    	
    	$num = M("user_activity")->where(array('act_id'=>$back['act_id'],'user_id'=>$back['user_id']))->order('create_time desc')->find();

    	//M("activity")->where(array('id'=>$back['act_id']))->setDec('participants',$num['num']); 	
    	
    	$back = M("order_back")->where(array('id'=>$id))->save(array('status'=>1,'create_time'=>time()));
